...
Foam::forces::forcesMoments Foam::forces::calcForcesMoment() const
{
    ...
    else
    {
        ...
        forAllConstIter(labelHashSet, patchSet_, iter)
        {
            ...
            //creat list
            List<Tuple2<scalar, scalar>> pProfile_top;

            List<Tuple2<scalar, scalar>> pProfile_bottom;
            ///////
            ...
            forAll(Md, faceI) {
                ...
                //Info<< "sAface: "<< Sfb[patchi][faceI] << nl << endl;          
            ...
            autoPtr<OFstream> pFilePtr_top;

            autoPtr<OFstream> pFilePtr_bottom;

            fileName forcesDir;

            word startTimeName = obr_.time().timeName();
                //obr_.time().timeName(obr_.time().startTime().value());              
            //Info << startTimeName << nl << endl;

            forcesDir = obr_.time().path()/name_/startTimeName;

            //Info << forcesDir << nl << endl;

            //Info << obr_.time().path() << nl << endl; is the case path

            mkDir(forcesDir);

            pFilePtr_top.reset(new OFstream(forcesDir/(type() + "_top.dat")));
            pFilePtr_bottom.reset(new OFstream(forcesDir/(type() + "_bottom.dat")));           
            ...
            forAll(pProfile_top, index) {
                pFilePtr_top() << pProfile_top[index].first() << tab << pProfile_top[index].second() << endl;
            }

            forAll(pProfile_bottom, index) {
                pFilePtr_bottom() << pProfile_bottom[index].first() << tab << pProfile_bottom[index].second() << endl;
            }
            //////////////////////
        }
    }
